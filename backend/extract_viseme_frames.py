...
def apply_scale(frame, scale_factor):
    ...
    resized = cv2.resize(frame, (new_w, new_h), interpolation=cv2.INTER_AREA if scale_factor < 1.0 else cv2.INTER_CUBIC)
    if scale_factor < 1.0:
        # Shrunk: pad back to original canvas size, replicating the plain
        # background so there's no visible seam at the border.
        pad_top = (h - new_h) // 2
        ...
        # Grew: center-crop back to original canvas size.
        y0 = (new_h - h) // 2
        x0 = (new_w - w) // 2
    ...
    for viseme_name, center_t in config.items():
        ...
        for i, off in enumerate(offsets):
            ...
            if frame is None:
                ...
                continue
            frame = apply_scale(frame, args.scale_factor)
...
def main():
    ...
    e.add_argument("--scale-factor", type=float, default=1.0, help="Shrink (e.g. 0.89) or grow the character before cropping, to match character size across two different source videos.")
